...
from .models import Appointment, Business, BusinessNotification, BusinessPage, Customer, Service, WorkingDay, WorkingHours
...
class AppointmentTests(TestCase):
	...
	def test_owner_can_list_and_cancel_future_appointment(self):
		...
		list_response = self.client.get(f'/appointments/?date={appointment.start_datetime.date().isoformat()}')
...
class WorkingHoursTests(TestCase):
	...
	def test_working_hours_are_isolated_between_businesses(self):
		other_user = User.objects.create_user(username='maria', password='Senha-forte-123')
		other_business = Business.objects.create(owner=other_user, name='Studio Maria')
		other_monday = WorkingDay.objects.create(
			business=other_business,
			day_of_week=0,
		)
		WorkingHours.objects.create(
			working_day=other_monday,
			start_time='09:00',
			end_time='17:00',
		)
		...
		self.client.post(
			'/working-hours/',
			self.schedule_payload({0: [('10:00', '16:00')]}),
		)
